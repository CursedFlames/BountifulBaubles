...
public class ItemlessRender<T extends EntityTerrariaThrowable> extends Render<T> {
	...
	public void doRender(T entity, double x, double y, double z, float entityYaw, float partialTicks) {
		...
		if (!this.bindEntityTexture(entity))
			BountifulBaubles.logger.fatal("Failed to get texture for bees! Extreme weirdness with rendering may ensue!");
		...
		bufferbuilder.pos(-0.25D, -0.25D, 0.0D).tex(0.0D, 1.0D).normal(0.0F, 1.0F, 0.0F).endVertex();
