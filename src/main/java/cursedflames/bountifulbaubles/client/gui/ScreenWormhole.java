...
//		int id = 0;
		for (int i = 0, j = 2; i<8; i++, j += 24) {
			addButton(new Button(guiLeft, guiTop+j, 80, 20,
					new StringTextComponent(""), captureNameButton(i*2)));
			addButton(new Button(guiLeft+WIDTH/2, guiTop+j, 80, 20,
					new StringTextComponent(""), captureNameButton(i*2+1)));
		}
		...
		TranslationTextComponent prev = new TranslationTextComponent(BountifulBaubles.MODID+".gui.misc.prev_page");
		TranslationTextComponent next = new TranslationTextComponent(BountifulBaubles.MODID+".gui.misc.next_page");
	...
	private void pageChange(int page) {
		for (int i = 0; i<16; i++) {
			...
			if (listIndex<container.targets.size()) {
				...
				if (target!=null) {
					button.setMessage(new StringTextComponent(target.getName()));
				}
